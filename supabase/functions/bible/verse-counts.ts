 ...
 * Verse Counts
 *
 * Expected verse counts per chapter for all 66 books.
 * Used for validation during parsing to catch errors.
 */
 ...
 * @param ref - Normalized reference like "Genesis 1" or "1 Samuel 13"
 * @returns Expected verse count or null if unknown
 */
export function getExpectedVerseCount(ref: string): number | null {
  // Parse "Genesis 1" or "1 Samuel 13" format
  ...
  if (!counts || chapter < 1 || chapter > counts.length) return null;

  return counts[chapter - 1];
}

/**
 * Find which verse numbers are missing from parsed results
 */
export function findMissingVerses(
  verses: Record<string, string>,
  expectedCount: number
): string {
  const parsed = new Set(Object.keys(verses).map((v) => parseInt(v, 10)));
